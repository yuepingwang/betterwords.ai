...
export function SiteHeader({ landing = false, clear = false, extra = null, extraWidth = 1280, onLogo, onNav, onStart }) {
  ...
  React.useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const hero = document.querySelector('.bw-v3 .grad-daybreak')
  ...
  return (
    <header style={shell}>
      ...
      <div style={{ width: '100%', padding: '0 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 68 }}>
        ...
        {/* nav links + CTA are landing-only; app screens keep the mark plus
            the account control (which renders nothing when accounts aren't
            configured — see lib/auth.jsx) */}
        {landing ? (
      ...
      {/* per-screen control + account chip, overlaid so they right-align with
          the page content container instead of the viewport-hugging bar above */}
      {!landing && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 68, pointerEvents: 'none' }}>
          <div style={{ maxWidth: extraWidth, height: '100%', margin: '0 auto', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12 }}>
            {extra && <span style={{ pointerEvents: 'auto', display: 'inline-flex' }}>{extra}</span>}
