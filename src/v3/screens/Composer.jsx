...
const GLYPHS = '/ds-v3/assets/glyphs'
...
export default function Composer() {
  ...
  const { Badge, Sparkle, Button, Icon, DraftPanel } = DS2
  ...
  // The header help button (V3App) bumps tourAsk to replay the tour.
  useEffect(() => {
    ...
    <main>
      ...
      >
        ...
        <DraftPanel
          ...
          title={`Option ${String(state.selectedIdx + 1).padStart(2, '0')}: ${strat.name}`}
          stance={strat.level}
          stanceLabel={stanceLabel(strat.level)}
          recommended={!!strat.recommended}
          shadeSrc="/ds-v3/assets/glass-shade.png"
          letterRef={letterRef}
          fields={[
            { label: 'To', value: recipientLabel(scenario) },
            { label: 'Re', value: strat.subject },
          ]}
          footer={addPanel}
        >
            ...
            <div ref={toolsRef} className="bw-cmp-tools">
              ...
              <span ref={addWrapRef} style={{ display: 'inline-flex', marginLeft: 4 }}>
                <button className="bw-cmp-addbtn" onClick={toggleAdd}>
                  <img src="/ds-v3/assets/glyphs/logo-star.svg" alt="" width={12} height={12} style={{ display: 'block' }} />
                  Add Something Else
                </button>
              </span>
            </div>
