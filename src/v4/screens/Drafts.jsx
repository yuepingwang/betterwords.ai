...
// Stance + Recommended tags share the composer's badge styling
// (.bw-cmp4-badge in Composer.css — white pill, blue text; gradient text
// with the sparkle for RECOMMENDED), so they read the same on both screens.
function StanceBadge({ st }) {
  return <span className="bw-cmp4-badge">{stanceLabel(st.level)}</span>
}

function Rec({ label = 'Recommended' }) {
  return (
    <span className="bw-cmp4-badge">
      <img src="/ds-v4/assets/glyphs/cmp4/rec-sparkle.svg" alt="" style={{ width: 10, height: 10 }} />
      <span className="bw-cmp4-grad-text">{label}</span>
    </span>
  )
}
