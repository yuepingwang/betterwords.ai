...
export async function evaluateLetter({ scenarioId, strategy, paras, convo = '' }) {
  ...
  try {
      ...
      [
        {
          ...
          content:
            ...
            '"pros": array of 2-3 objects { "text": short phrase (max ~8 words) naming what THIS wording does well, ' +
            '"quote": the exact passage (3-12 words) copied VERBATIM from the message that best shows it — every character must match the message }, ' +
            '"cons": array of 1-2 objects { "text": short phrase naming a real risk or cost of THIS wording, ' +
            '"quote": the exact passage copied VERBATIM from the message that creates that risk }, ' +
    ...
    const parsed = parseJson(content)
    // Each pro/con carries the verbatim excerpt it's grounded in, so the UI
    // can quote it and highlight it in the letter. Plain strings (older
    // shapes, static fallbacks) still parse — their quote is just null.
    const phrases = (arr, max) =>
      Array.isArray(arr) && arr.length
        ? arr
            .map((p) =>
              typeof p === 'string'
                ? { text: p.trim(), quote: null }
                : p && typeof p === 'object'
                  ? { text: String(p.text || '').trim(), quote: p.quote ? String(p.quote).trim() : null }
                  : null
            )
            .filter((p) => p && p.text)
            .slice(0, max)
